...
class AdminController extends BaseController {
    ...
    public function viewControlPost(){
        ...
        $t = DB::table('user')
            ...
            ->select('user.email', 'post.title', 'post.link_file', 'post.content', 'category.title as cat_title', 'user.id as user_id', 'post.id as post_id', 'category.id as cat_id')
            ->get();

        $tempUser = new User();
        $user = $tempUser->getUserByToken();

        return View::make('admin.controlpost', array('allRows' => $t, 'admin' => $user->id));
    ...
    public function removePost(){
        ...
        if (Request::ajax()){
            ...
            if ($user == null){
                ...
                $tempPost = new Post();
                $tempPost->removePost($data['postId']);
                return Response::json(array('status' => '200'));
            }
        }
    }
